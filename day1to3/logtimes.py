...
def main():
    ...
    for index, a in enumerate(loglines):
        ...
        if info[0:18] == 'Shutdown initiated':
            print("Logline {} is a shutdown".format(index))
            list_shutdowns.append((index, info))

    print(list_shutdowns)

    for index_shutdowns, info_shutdown in enumerate(list_shutdowns):
        print(index_shutdowns, info_shutdown)


    first_shutdown_index = (list_shutdowns[0][0])
    last_shutdown_index = (list_shutdowns[len(list_shutdowns)-1][0])
